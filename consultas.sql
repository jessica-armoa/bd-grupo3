...
SELECT
	...
	DF.CANTIDAD,
	DF.PRECIO_UNITARIO
FROM FACTURAS AS F (SELECT NOMBRE
FROM PERSONAS) P
(SELECT CLIENTE_ID
FROM CLIENTES) C
(SELECT DESCRIPCION
...
AND CLIENTES.PERSONA_ID = PERSONAS.PERSONA_ID
AND FACTURAS.FACTURA_ID = DETALLES_FACTURAS.FACTURA_ID
AND SERVICIOS_REALIZADOS.SERVICIO_REALIZADO_ID = FACTURAS.SERVICIO_REALIZADO_ID
AND PRODUCTOS.PRODUCTO_ID = DETALLES_FACTURAS.PRODUCTO_ID
AND (F.FECHA BETWEEN '05-11-2023' AND '10-11-2023')
AND (C.CLIENTE_ID BETWEEN 1 AND 3)
GROUP BY 	P.NOMBRE,
	C.CLIENTE_ID,
	F.FECHA,
	SR.DESCRIPCION,
